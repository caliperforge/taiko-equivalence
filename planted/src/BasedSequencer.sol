...
/// @title BasedSequencer (PLANTED twin) - Taiko based-sequencing ordering mirror.
///
...
contract BasedSequencer {
    ...
    function finalize() external returns (bytes32[] memory order) {
        require(!finalized, "already finalized");
        finalized = true;
        // PLANTED: sort by priorityFee descending. An honest based
        // proposer MUST preserve insertion order. Sorting by priority fee
        // is the canonical "sequencer reorder for MEV" violation that
        // based-sequencing is supposed to make impossible.
        ProposedTx[] memory sorted = queue;
        ...
        for (uint256 k = 0; k < sorted.length; k++) {
            order[k] = sorted[k].txHash;
        }
    }
